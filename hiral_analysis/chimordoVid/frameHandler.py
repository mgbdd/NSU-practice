...
class frame_handler:
    def __init__(self):
        self.x_start = 5                                    #индекс с которого начинаются x координаты landmarks в dataframe
        self.num_landmarks = 68        
        self.y_start = self.x_start + self.num_landmarks     #индекс с которого начинаются y координаты landmarks в dataframe
    ...
    def rotate_tensor(self, frame_tensor, landmarks):
        ...
        if not np.isnan(roll):
            ...
            nan_flag_rot = True
            print("Cannot see vertical alignment landmarks, skipping frame")
    ...
    def mirror_faces(self, orig_tensor, landmarks):
        ...
        return left_mirrored_tensor * 255, right_mirrored_tensor * 255, nan_flag_mir
